...
const char layers[][KEYS] PROGMEM = {
  ...
  { // Mods Fn
    ...
    KEY_TAB, '6', '7', '8', '9', '0',
    KEY_LAY2, '\\', '=', '-', '`',
    KEY_LEFT_ALT, KEY_LEFT_CTRL, KEY_FN3,
  ...
  { // Fn3
    ...
    KEY_BACKSPACE, ' ', KEY_SYM, KEY_SYM, KEY_SYM,
    ' ', KEY_FN, KEY_RIGHT_ALT
  },
  { // Fn4
...
String usyms(byte k) {
  if (k == 35 && os <= LINUX) return os == LINUX_COMPOSE ? "<<" : "00ab";
  else if (k == 35 && os == WINDOWS) return "0171";

  else if (k == 36 && os <= LINUX) return os == LINUX_COMPOSE ? ">>" : "00bb";
...
void release(byte k) {
  ...
  if (code == mlast && (IS_MODS(code) || IS_FN(code))) { // Mods click
